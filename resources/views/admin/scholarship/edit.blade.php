...
@section('content')
    <h1 class="text-2xl mb-4">Edit Scholarship</h1>

    <form action="{{ route('scholarship.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 max-w-2xl">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm">Current Picture</label>
            <img src="{{ $item->picture ? asset('storage/' . $item->picture) : asset('img/home/scholarships/default.png') }}" class="w-48 h-32 object-cover rounded mb-2">
        </div>
        ...
        <div>
            ...
            <input type="date" name="deadline" class="w-full border p-2" value="{{ old('deadline', optional($item->deadline)->format('Y-m-d')) }}">
